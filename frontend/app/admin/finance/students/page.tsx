...
export default function StudentFinance() {
  ...
  const downloadReceipt = async (paymentDocId: string, paymentId: number) => {
    ...
    try {
      ...
      let qrContent = `2CS COMPLEXE SCOLAIRE\nFacture: ${invoiceNumber}\nÉlève: ${studentName}\nID: ${studentUserId}\nMontant: ${Number(pay.amount).toLocaleString()} GNF`;
      try {
      ...
      doc.setFont('Helvetica', 'normal');
      doc.text(`Montant total collecté: ${Number(pay.amount || 0).toLocaleString()} GNF`, 15, finalY + 28);
      doc.text(`Solde restant de la facture: ${Number(remainingBalance).toLocaleString()} GNF`, 15, finalY + 35);
  ...
  const downloadStatement = async (studentId: number | null) => {
    ...
    try {
      ...
      doc.setFont('Helvetica', 'normal');
      doc.text(`Total facturé: ${Number(data.totalInvoiced || 0).toLocaleString()} GNF`, 125, 70);
      doc.text(`Total payé:   ${Number(data.totalPaid || 0).toLocaleString()} GNF`, 125, 77);
      doc.text(`En souffrance:  ${Number(data.outstandingBalance || 0).toLocaleString()} GNF`, 125, 84);
      ...
      <Dialog open={isPaymentOpen} onOpenChange={(open) => { setIsPaymentOpen(open); if (!open) { setEditingPayment(null); setSelectedInvoiceId(''); setPaymentClassId('NONE'); } }}>
        <DialogContent className="max-w-md bg-white rounded-3xl">
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="space-y-1">
              <label className="text-xs font-black uppercase text-slate-400">Montant décaissé (GNF)</label>
              <Input
                type="number"
                value={paymentAmount || ''}
