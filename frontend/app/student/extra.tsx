...
export default function StudentDashboard() {
   ...
   const fetchDashboardData = async () => {
      try {
         ...
         console.error("Erreur de synchronisation du tableau de bord", error);
      } finally {
         setLoading(false);
      }
   };
   ...
   if (loading) {
      return (
         <div className="h-screen flex flex-col items-center justify-center gap-4 bg-[#fcfcfd]">
            ...
            <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">Synchronisation du profil...</p>
         </div>
      );
   }
      ...
      <div className="min-h-screen bg-[#fcfcfd] p-6 lg:p-10 space-y-8">
         ...
         <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
            {[
               ...
               { label: 'Note moyenne', value: stats.averageGrade, icon: Award, color: 'text-indigo-600', bg: 'bg-indigo-50' },
            ].map((item, i) => (
               <motion.div
                  key={item.label}
                  ...
                  <Card className="border border-slate-100 md:hover:border-primary duration-500 shadow-sm rounded-4 overflow-hidden bg-white group hover:shadow-md transition-all">
                     <CardContent className="p-6">
                        <div className="flex justify-between items-start">
                           ...
                           <Badge variant="outline" className="text-[10px] border-slate-100 uppercase font-black text-slate-400">En direct</Badge>
                        </div>
                        <div className="mt-4">
         ...
         <div className="grid gap-6 lg:grid-cols-3">
            ...
            <Card className="lg:col-span-2 border border-slate-100 md:hover:border-primary duration-500 transition-colors shadow-sm rounded-[2.5rem] bg-white p-8">
               <div className="flex items-center justify-between mb-8">
                  <div>
                     <h2 className="text-xl font-black text-slate-900 uppercase tracking-tight italic">Pulse de présence</h2>
                     <p className="text-xs text-slate-400 font-bold uppercase tracking-widest mt-1">Suivi de cohérence</p>
                  </div>
                  <TrendingUp className="text-emerald-500" />
               </div>
               <div className="h-[250px] w-full">
                  <ResponsiveContainer width="100%" height="100%">
                     <AreaChart data={[{ day: 'Lun', value: 85 }, { day: 'Mar', value: 90 }, { day: 'Mer', value: 70 }, { day: 'Jeu', value: stats.attendance }, { day: 'Ven', value: 100 }]}>
                        <defs>
            ...
            <div className="space-y-6">
               <Card className="border border-slate-800 md:hover:border-primary duration-500 transition-colors shadow-sm rounded-[2.5rem] bg-slate-900 text-white p-8 overflow-hidden relative group">
                  <div className="relative z-10">
                     <h2 className="text-lg font-black uppercase tracking-widest mb-4 italic">Rappel d'examen</h2>
                     {extras.nextExam ? (
               ...
               <Card className="border border-slate-100 md:hover:border-primary duration-500 transition-colors shadow-sm rounded-[2.5rem] bg-white p-8">
                  <h2 className="text-xs font-black uppercase text-slate-400 tracking-widest mb-6">Activités récentes</h2>
                  <div className="space-y-6">
                     {extras.recentActivities.length > 0 ? (
                        ...
                        <div className="text-center py-4">
                           <p className="text-[10px] font-bold text-slate-300 uppercase tracking-widest italic">Aucune mise à jour récente</p>
                        </div>
                     )}
                  </div>
               </Card>
