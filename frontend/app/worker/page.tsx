...
import {
  ...
  Activity, UserCircle, RefreshCw, Wallet, AlertCircle
} from 'lucide-react';
...
export default function WorkerDashboard() {
  ...
  const downloadPayslip = async (rec: any) => {
    ...
    try {
      ...
      // Calculate disbursed for this record
      const recordPayments = salaryPayments.filter(
        p => p.salaryRecordId === rec.id && p.status === 'APPROVED'
      );
      ...
      doc.text(`${SCHOOL_CONFIG.subtitle} — EMPLOYEE SALARY PAYSLIP`, 15, 30);
      ...
      doc.text(`Role: ${profile?.schoolRole || 'WORKER'}`, 120, 94);
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center min-h-screen gap-3">
        ...
        <p className="font-bold text-slate-500 uppercase text-sm tracking-wide">Syncing Support Staff Portal...</p>
      </div>
    );
  }
    ...
    <div className="flex max-md:flex-col min-h-screen">
      ...
      <aside className="hidden md:flex md:flex-col md:w-72 border-r border-slate-100 h-screen sticky top-0 z-50 bg-white">
        ...
        <div className="flex items-center gap-3 px-6 h-20 border-b border-slate-50">
          ...
          <div>
            ...
            <span className="text-[9px] font-black text-blue-600 uppercase tracking-widest">Support Portal</span>
          </div>
        </div>
              ...
              <div className="overflow-hidden">
                ...
                <div className="flex items-center gap-1.5 mt-0.5">
                  ...
                  <span className="text-[9px] text-emerald-400 font-black uppercase tracking-widest">
                    {profile?.schoolRole || 'WORKER'} Session
                  </span>
                </div>
              </div>
            </div>
      ...
      <div className="md:hidden flex items-center justify-between p-4 bg-white border-b sticky top-0 z-50">
        <div className="flex items-center gap-2">
          ...
          <span className="font-black text-base italic uppercase tracking-tight">Support Portal</span>
        </div>
      ...
      <main className="flex-1 p-6 md:p-8 space-y-8 bg-slate-50/50">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl md:text-3xl font-black tracking-tight text-slate-900 italic uppercase">Support Staff Portal</h1>
            <p className="text-sm text-slate-500 font-medium mt-1">
              Welcome back, <span className="font-extrabold text-blue-600">{profile?.name || profile?.username}</span>! Track your salary statements and payslips.
            </p>
          </div>
        ...
        {salaryPayments.length > 0 && (
          <Card className="border-0 shadow-xl bg-white rounded-3xl overflow-hidden">
            <CardHeader className="px-6 py-5 border-b border-slate-50">
              <CardTitle className="text-sm font-black uppercase tracking-wider text-slate-500 flex items-center gap-2">
                <CreditCard className="w-4 h-4 text-emerald-500" /> Payment Disbursement History
              </CardTitle>
            </CardHeader>
            <CardContent className="p-0">
              <Table>
                <TableHeader>
                  <TableRow className="bg-slate-50/50">
                    <TableHead className="font-bold text-slate-700">Payment Ref</TableHead>
                    <TableHead className="font-bold text-slate-700">Period</TableHead>
                ...
                <TableBody>
                  {salaryPayments.map((pay: any) => (
                    <TableRow key={pay.id} className="hover:bg-slate-50/50">
                      <TableCell className="font-bold text-slate-900 text-xs">{pay.paymentNumber}</TableCell>
                      <TableCell className="text-sm text-slate-600">{pay.month} {pay.year}</TableCell>
                      <TableCell className="font-black text-emerald-600">{fmtGNF(Number(pay.amount || 0))} <span className="text-[10px] text-emerald-300">GNF</span></TableCell>
                      <TableCell className="text-xs text-slate-500 font-semibold">{pay.paymentMethod || '—'}</TableCell>
