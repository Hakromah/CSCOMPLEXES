...
import { Loader2, Bus, MapPin, Clock, Phone, User, ChevronRight } from 'lucide-react';
...
export default function ChildTransportPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    const fetchTransportData = async () => {
      try {
        const [studentRes, transportRes] = await Promise.all([
          api.get(`/parent/children/${id}`),
          api.get(`/parent/transport/${id}`).catch((err) => {
            if (err.response?.status === 404) return { data: null };
            throw err;
          }),
        ]);
        setStudent(studentRes.data);
        setAssignment(transportRes.data);
      } catch (err) {
  ...
  return (
    <div className="p-6 lg:p-10 min-h-screen space-y-8 bg-[#f8fafc]">
      ...
      <div className="flex items-center gap-2 text-xs text-slate-400 font-medium">
        ...
        <span>/</span>
        <Link href={`/parent/children/${id}`} className="hover:text-primary transition-colors">{studentName}</Link>
        <span>/</span>
      ...
      <header className="space-y-2">
        ...
        <p className="text-slate-400 font-bold text-[10px] uppercase tracking-widest">
          Active route, timings, driver details and vehicle info for {studentName}
        </p>
      </header>
      ...
      {assignment ? (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <Card className="lg:col-span-2 border border-slate-100 rounded-3xl overflow-hidden bg-white shadow-sm hover:shadow-md transition-shadow p-8 space-y-8">
            ...
            {assignment.transportFee > 0 && (
              <div className="bg-blue-50/50 border border-blue-100 rounded-2xl p-6">
                <span className="text-[9px] font-black uppercase tracking-widest text-primary block mb-1">Monthly Transport Fee</span>
                <p className="text-base font-black text-slate-900">{assignment.transportFee.toLocaleString()} GNF</p>
              </div>
            )}
        ...
        <Card className="border-2 border-dashed border-slate-200 rounded-3xl bg-white p-12 text-center max-w-2xl mx-auto space-y-4">
          ...
          <p className="text-slate-400 text-xs font-bold leading-relaxed max-w-md mx-auto">
            This student is not currently assigned to any active school transport route. Contact administration to register for transit services.
          </p>
        </Card>
      )}
    </div>
