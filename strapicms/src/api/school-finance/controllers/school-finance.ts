...
export default {
  ...
  async getPaymentMethodsSettings(ctx: any) {
    try {
      ...
      if (settings) {
        return (ctx.body = {
          bankName: settings.bankName || 'Banque Centrale / Vista Bank Guinée',
          accountHolder: settings.accountHolder || '2CS COMPLEXE SCOLAIRE',
          rib: settings.rib || 'GN04 0001 2345 6789 0123 45',
          branchCode: settings.branchCode || '01001',
          swift: settings.swift || 'VISTGNCON',
          bankInstructions: settings.bankInstructions || 'Mentionnez impérativement le N° de Facture et votre ID Élève en référence de votre virement pour un rapprochement comptable immédiat.',
          ...
          orangeMoneyInstructions: settings.orangeMoneyInstructions || 'Composez le code USSD ci-dessus pour effectuer le paiement direct sécurisé.',
          ...
          mtnMoMoInstructions: settings.mtnMoMoInstructions || 'Paiement marchand via le code court MTN Mobile Money.',
          isMtnMoMoActive: settings.isMtnMoMoActive !== undefined ? Boolean(settings.isMtnMoMoActive) : true,
          cashierLocation: settings.cashierLocation || 'Bâtiment Administratif, RDC',
          cashierHours: settings.cashierHours || 'Lundi au Vendredi: 08h00 — 16h00',
          isCashierActive: settings.isCashierActive !== undefined ? Boolean(settings.isCashierActive) : true,
          contactEmail: settings.contactEmail || 'accounts@2cscomplexes.com',
          contactPhone: settings.contactPhone || '+224 620 00 00 00',
          additionalNotes: settings.additionalNotes || 'Conservez précieusement vos reçus de versement pour tout contrôle administratif.'
        });
      }
